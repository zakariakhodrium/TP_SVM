...
symlist = ['o', 's']  # Add more if there are more classes
...
def frontiere(f, X, y, w=None, step=50, alpha_choice=1, colorbar=True, samples=True):
    ...
    xx, yy = np.meshgrid(
        np.arange(min_tot0, max_tot0, delta0 / step),
    ...
    if samples is True:
        for i, label in enumerate(y):
            ...
            plt.scatter(
                X[i, 0],
                X[i, 1],
                color=color_blind_list[label_num],
                s=80,
                marker=symlist[label_num],
            )
